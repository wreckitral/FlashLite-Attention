"""
Profile Occupancy Metrics - Collects data for Table 4
Metrics: Theoretical Occupancy (%), Achieved Occupancy (%)

This script runs the kernels and saves profiles that need to be analyzed with:
ncu --csv --metrics <metrics> python profile_occupancy.py

Or use the generated shell script: run_occupancy_profile.sh
"""
import torch
...
def generate_ncu_script():
    ...
    metrics = [
        ...
        "launch__occupancy_limit_blocks",                      # Theoretical occupancy (blocks)
        "launch__occupancy_limit_warps",                       # Theoretical occupancy (warps)
...
# Auto-generated script to profile occupancy metrics (Table 4)
# Run this with: bash run_occupancy_profile.sh
...
echo "======================================"
echo "Profiling Occupancy Metrics (Table 4)"
echo "======================================"
...
ncu --csv \\
    ...
    --export $RESULTS_DIR/pytorch_occupancy \\
    python $SCRIPT_DIR/profile_occupancy.py pytorch
...
ncu --csv \\
    --metrics {metrics_str} \\
    --target-processes all \\
    --export $RESULTS_DIR/p0_occupancy \\
    python $SCRIPT_DIR/profile_occupancy.py p0
...
ncu --csv \\
    --metrics {metrics_str} \\
    --target-processes all \\
    --export $RESULTS_DIR/p1_occupancy \\
    python $SCRIPT_DIR/profile_occupancy.py p1
...
ncu --csv \\
    --metrics {metrics_str} \\
    --target-processes all \\
    --export $RESULTS_DIR/p2_occupancy \\
    python $SCRIPT_DIR/profile_occupancy.py p2
...
"""
    ...
    script_path = Path("run_occupancy_profile.sh")
...
def main():
    print("="*80)
    print("OCCUPANCY PROFILING - Table 4")
    print("="*80)
    ...
    if len(sys.argv) > 1:
        ...
        if kernel_choice == "pytorch":
            ...
            print("Usage: python profile_occupancy.py [pytorch|p0|p1|p2]")
            sys.exit(1)
        ...
        print("   ncu --csv --metrics <metrics> python profile_occupancy.py [pytorch|p0|p1|p2]")
        print()
