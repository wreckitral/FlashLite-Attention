"""
Profile Shared Memory Metrics - Collects data for Table 3
Metrics: Shared Memory Usage (KB), Uncoalesced Accesses (%), Bank Conflicts

This script runs the kernels and saves profiles that need to be analyzed with:
ncu --csv --metrics <metrics> python profile_shared_memory.py

Or use the generated shell script: run_shared_memory_profile.sh
"""
import torch
...
def generate_ncu_script():
    ...
    metrics = [
        ...
        "l1tex__data_bank_conflicts_pipe_lsu",           # Bank conflicts
...
# Auto-generated script to profile shared memory metrics (Table 3)
# Run this with: bash run_shared_memory_profile.sh
...
echo "=========================================="
echo "Profiling Shared Memory Metrics (Table 3)"
echo "=========================================="
...
ncu --csv \\
    ...
    --export $RESULTS_DIR/pytorch_shared_memory \\
    python $SCRIPT_DIR/profile_shared_memory.py pytorch
...
ncu --csv \\
    --metrics {metrics_str} \\
    --target-processes all \\
    --export $RESULTS_DIR/p0_shared_memory \\
    python $SCRIPT_DIR/profile_shared_memory.py p0
...
ncu --csv \\
    --metrics {metrics_str} \\
    --target-processes all \\
    --export $RESULTS_DIR/p1_shared_memory \\
    python $SCRIPT_DIR/profile_shared_memory.py p1
...
ncu --csv \\
    --metrics {metrics_str} \\
    --target-processes all \\
    --export $RESULTS_DIR/p2_shared_memory \\
    python $SCRIPT_DIR/profile_shared_memory.py p2
...
"""
    ...
    script_path = Path("run_shared_memory_profile.sh")
...
def main():
    print("="*80)
    print("SHARED MEMORY PROFILING - Table 3")
    print("="*80)
    ...
    if len(sys.argv) > 1:
        ...
        if kernel_choice == "pytorch":
            ...
            print("Usage: python profile_shared_memory.py [pytorch|p0|p1|p2]")
            sys.exit(1)
        ...
        print("   ncu --csv --metrics <metrics> python profile_shared_memory.py [pytorch|p0|p1|p2]")
        print()
